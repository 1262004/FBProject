...
// THE MANAGEMENT > PRODUCTS PAGE

directivesModule.directive('managementProductsContent', ['$http', '$rootScope', function($http, $rootScope) {
    return {
        restrict: 'E',
        templateUrl: '/views/management/management-products-content.html',
        link: function (scope, element, attrs) {
            scope.productsTableData = null;
            scope.selectedApp = null;
            scope.selectedAppReporters = null;
            scope.selectedAppReporter = null;

            scope.addNewProductBtnHandler = function($event){
                var productName = $("#product-name").val();
                var productURL = $("#product-url").val();
                $http.get('/app/add?name=' + productName + '&url=' + productURL).success(function(data) {
                        // this callback will be called asynchronously
                        // when the response is available
                        scope.productsTableData.push(data);
                        $('#add-new-product-modal').modal('hide');
                    });
            };

            scope.confirmDeleteProduct = function($event){
                var selectedAppIdString = angular.element(event.currentTarget).parent().parent().parent().find('td:first-child').text();
                var selectedAppID = Number(selectedAppIdString);
                for (var i = 0; i < scope.productsTableData.length; i++){
                    if (scope.productsTableData[i].appId == selectedAppID){
                        scope.selectedApp = scope.productsTableData[i];
                        break;
                    }
                }
            ...
            scope.deleteProductBtnHandler = function($event){
                $http.get('/app/delete?appId=' + scope.selectedApp.appId).success(function(response){
                    //console.log(response);
                    var index = scope.productsTableData.indexOf(scope.selectedApp);
                    scope.productsTableData.splice(index, 1);
                    $('#delete-product-modal').modal('hide');
                });
            };

            scope.viewEmbeddedCode = function($event){
                var selectedAppIdString = angular.element(event.currentTarget).parent().parent().parent().find('td:first-child').text();
                var selectedAppID = Number(selectedAppIdString);
                for (var i = 0; i < scope.productsTableData.length; i++){
                    if (scope.productsTableData[i].appId == selectedAppID){
                ...
                $('#view-embedded-code-modal').modal('show');
            };

            scope.startEditProduct = function($event){
                var selectedAppIdString = angular.element(event.currentTarget).parent().parent().parent().find('td:first-child').text();
                var selectedAppID = Number(selectedAppIdString);
                for (var i = 0; i < scope.productsTableData.length; i++){
                    if (scope.productsTableData[i].appId == selectedAppID){
            ...
            scope.editProductBtnHandler = function($event){
                var newProductName = $('#edit-product-name').val();
                var newProductTimeout = Number($('#edit-product-timeout').val());
                var newProductStatus = $("input[type='radio'][name='edit-product-status']:checked").data("status");
                var queryString = '/app/update?appId=' + scope.selectedApp.appId + '&name=' + newProductName + '&timeout=' + newProductTimeout + '&status=' + newProductStatus + '&registeredAt=' + scope.selectedApp.createdAt;
                $http.get(queryString)
                    .success(function(data){
                        for (var i = 0; i < scope.productsTableData.length; i++){
                            if (scope.productsTableData[i].appId == scope.selectedApp.appId){
                                scope.productsTableData[i].name = newProductName;
                                scope.productsTableData[i].timeout = newProductTimeout;
                                scope.productsTableData[i].timeout = newProductStatus;
            ...
            scope.viewListOfReporters = function($event){
                var selectedAppIdString = angular.element(event.currentTarget).parent().parent().parent().find('td:first-child').text();
                var selectedAppID = Number(selectedAppIdString);
                for (var i = 0; i < scope.productsTableData.length; i++){
                    if (scope.productsTableData[i].appId == selectedAppID){
                        scope.selectedApp = scope.productsTableData[i];
                        var selectedAppReportersQuery = '/app/reporter/list?appId=' + scope.selectedApp.appId;
                        $http.get(selectedAppReportersQuery).success(function(data){
                                scope.selectedAppReporters = data;
                                $('#view-reporters-list-modal').modal('show');
                            });
                        break;
                    }
                }
            ...
            scope.addNewReporterBtnHandler = function($event){
                var reporterName = $('#reporter-name').val();
                var selectedAppId = scope.selectedApp.appId;
                var queryString = '/app/reporter/add?appId=' + selectedAppId + '&username=' + reporterName;
                $http.get(queryString).success(function(data){
                    var selectedAppReportersQuery = '/app/reporter/list?appId=' + scope.selectedApp.appId;
                    $http.get(selectedAppReportersQuery).success(function(data) {
                        scope.selectedAppReporters = data;
                        $('#add-reporter-modal').modal('hide');
                    });
                });
            };

            scope.confirmDeleteReporter = function($event){
                var selectedUsername = angular.element(event.currentTarget).parent().parent().find('td:nth-child(2)').text();
                for (var i = 0; i < scope.selectedAppReporters.length; i++){
                    if (scope.selectedAppReporters[i].username == selectedUsername){
                        scope.selectedAppReporter = scope.selectedAppReporters[i];
                        break;
                    }
                }
                $('#delete-reporter-modal').modal('show');
            };

            scope.removeReporterBtnHandler = function($event){
                var selectedAppId = scope.selectedApp.appId;
                var queryString = '/app/reporter/remove?appId=' + selectedAppId + '&username=' + scope.selectedAppReporter.username;
                $http.get(queryString).success(function(data){
                    var selectedAppReportersQuery = '/app/reporter/list?appId=' + scope.selectedApp.appId;
                    $http.get(selectedAppReportersQuery).success(function(data) {
                        scope.selectedAppReporters = data;
                        $('#delete-reporter-modal').modal('hide');
                    });
                });
            };

            scope.confirmLeaveProduct = function($event){
                var selectedAppIdString = angular.element(event.currentTarget).parent().parent().parent().find('td:first-child').text();
                var selectedAppID = Number(selectedAppIdString);
                for (var i = 0; i < scope.productsTableData.length; i++){
                    if (scope.productsTableData[i].appId == selectedAppID){
                        scope.selectedApp = scope.productsTableData[i];
                        $('#reporter-leave-product-modal').modal('show');
                        break;
                    }
                }
            };

            scope.reporterLeaveProduct = function($event){
                var selectedAppId = scope.selectedApp.appId;
                var queryString = '/app/reporter/leave?appId=' + selectedAppId;
                $http.get(queryString).success(function(data){
                    $http.get('/app/list').success(function(data){
                        scope.productsTableData = data;
                        $('#reporter-leave-product-modal').modal('hide');
                    });
                });
            };
