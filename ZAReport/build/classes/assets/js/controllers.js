...
controllersModule.controller('ChatController', ['$scope', '$rootScope', '$http', 'ChatService', function($scope, $rootScope, $http, ChatService){
    ...
    $rootScope.$watchGroup(['userProfile', 'currentAppId'], function(){
        if ($rootScope.userProfile && $rootScope.userProfile.userId && $rootScope.currentAppId){
            $http.get('app/reporter/list?appId=' + $rootScope.currentAppId).success(function(data){
